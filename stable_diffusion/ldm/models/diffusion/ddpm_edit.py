...
from ldm.hfp import extract_high_freq_component, sobel_operator
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 scale_by_std=False,
                 deepspeed="",
                 disable_hfp_loss=True,
                 *args, **kwargs):
        self.disable_hfp_loss = disable_hfp_loss
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # Image.fromarray((((torch.clamp(x[0], -1, 1) + 1) / 2).cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)).save('x.jpg')
        if bs is not None:
            x = x[:bs]
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        # Image.fromarray((((torch.clamp(self.decode_first_stage(encoder_posterior.mode() * self.scale_factor)[0], -1, 1) + 1) / 2).cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)).save('x_rec.jpg')
        cond_key = cond_key or self.cond_stage_key
        xc = super().get_input(batch, cond_key)
        if self.model.diffusion_model.disable_dual_context:
            if bs is not None:
            ...
            assert len(xc["c_crossattn"]) == 2, ("Dual context attn requires two conditioning keys", len(xc["c_crossattn"]))
            inst_prompt = xc["c_crossattn"][0]
            flaw_prompt = xc["c_crossattn"][1]
            if bs is not None:
                flaw_prompt = flaw_prompt[:bs]
                inst_prompt = inst_prompt[:bs]
                xc["c_concat"] = xc["c_concat"][:bs]
            cond = {}

            random = torch.rand(x.size(0), device=z.device)
            prompt_mask = rearrange(random < 0.2, "n -> n 1 1")
            null_prompt = self.get_learned_conditioning([""])
            inst_prompt = torch.where(
                            prompt_mask, 
                            null_prompt, 
                            self.get_learned_conditioning(inst_prompt).detach()
                        ).unsqueeze(-1)
            prompt_mask = rearrange(random < 0.075, "n -> n 1 1")
            flaw_prompt = torch.where(
                            prompt_mask, 
                            null_prompt, 
                            self.get_learned_conditioning(flaw_prompt).detach()
                        ).unsqueeze(-1)
            
            cond["c_crossattn"] = [
                rearrange(
                    torch.cat([inst_prompt, flaw_prompt], -1), "b n d e -> (b e) n d"
                )
            ]
        ...
        if return_original_cond:
            out.append(xc)
        if not self.disable_hfp_loss:
            out.append(batch[k])
        return out


    @torch.no_grad()
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, skip_connect_hs=None):
        if predict_cids:
            if z.dim() == 4:
    ...
    def differentiable_decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, skip_connect_hs=None):
        if predict_cids:
            if z.dim() == 4:
                z = torch.argmax(z.exp(), dim=1).long()
            z = self.first_stage_model.quantize.get_codebook_entry(z, shape=None)
    ...
    def forward(self, batch, batch_idx, num_steps, *args, **kwargs):
        my_input, x_original = self.get_input(batch, self.first_stage_key), None
        if self.disable_hfp_loss:
            x, c = my_input
        else:
            x, c, x_original = my_input
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=x.device).long()
        ...
        loss, loss_dict = self.p_losses(x, c, t, x_original, *args, **kwargs)
    ...
    def get_hfp_loss(self, x_pred, x_original, t, lambdas=[0.001]):
        # Image.fromarray((((torch.clamp(x_pred[2], -1, 1) + 1) / 2).cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)).save('x_pred.jpg')
        fft_pred, sobel_pred = self._get_hf_map(x_pred)
        fft_original, sobel_original = self._get_hf_map(x_original.to(x_pred.device))
        
        fft_loss = F.mse_loss(fft_pred, fft_original, reduction='none')
        sobel_loss = F.mse_loss(sobel_pred, sobel_original, reduction='none')
        loss = (fft_loss + sobel_loss) / 2
        
        loss = torch.mean(torch.flatten(loss, 1), dim=1)
        # get the weighting condition on the time step
        loss = loss * torch.exp(-lambdas[0] * t)
    ...
    def p_losses(self, x_start, cond, t, x_original=None, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond)
        
        # pred_x0 = self.pred_xstart_from_eps(x_t=x_noisy, t=t, epilson=model_output)
        # x_pred = self.decode_first_stage(pred_x0)
        # x_pred = torch.clamp(x_pred, min=-1.0, max=1.0)
        ...
        # loss_hfp = self.get_hfp_loss(x_pred, x_original, t)
        # loss = loss + loss_hfp
        # loss_dict.update({f'{prefix}/loss_hfp': loss_hfp})
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if x_T is None:
            img = torch.randn(shape, device=x_T.device)
        else:
            img = x_T
        intermediates = []
        if cond is not None:
        ...
        for i in iterator:
            ts = torch.full((b,), i, device=cond.device, dtype=torch.long)
            if self.shorten_cond_schedule:
